 ...
 * Principio: Single Responsibility - Solo maneja UI de autenticación
 */
...
function LoginPage() {
  ...
  const [registeredUsers] = useState([
    { email: 'admin@magneto.com', password: 'admin123', role: 'admin' as const, name: 'Administrador' },
    { email: 'usuario@test.com', password: 'usuario123', role: 'user' as const, name: 'Usuario' }
  ]);
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      const loggedInUser = login({ username: mockUsername, password: mockPassword });
      if (loggedInUser) {
